...
# Create/refresh remove script from published template (single source of truth)
REMOVE_SCRIPT_URL="${MAICRO_REMOVE_SCRIPT_URL:-https://raw.githubusercontent.com/bloxez/maicroverse/main/maicro-install/remove.sh}"
if ! curl -fsSL "$REMOVE_SCRIPT_URL" -o "${DATA_DIR}/remove.sh"; then
    printf "${RED}ERROR: Failed to download remove script from:${NC} %s\n" "$REMOVE_SCRIPT_URL"
    exit 1
fi
